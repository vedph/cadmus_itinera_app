...
1. `npm run build-all`
2. `ng build`
3. `docker build . -t vedph2020/cadmus-itinera-app:1.0.41 -t vedph2020/cadmus-itinera-app:latest` (replace with the current version).
...
3. build a new image for production: `docker build . -t vedph2020/cadmus-itinera-app:1.0.41-prod`. The production version is labeled like this one, with `-prod` suffix.
